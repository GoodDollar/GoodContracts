...
/**
 * BancorFormula code copied from
 * the BancorProtocol repo to give
 * full functionality and to prevent
 * need to re-invent the wheel.
 *
 * Had to then break out ExpArray as
 * a separate contract to allow deployment
 * to Ropsten. Removed unused functions.
 *
 * original source: 
 ...
 * and
 *
 * https://github.com/bancorprotocol/contracts/blob/master/solidity/contracts/utility/Utils.sol
 *
...
contract BancorFormula {
    ...
    ExpArray public expArray;

    constructor(address _exp) public {
        expArray = ExpArray(_exp);
    }
    ...
    */
    function calculatePurchaseReturn(uint256 _supply, uint256 _connectorBalance, uint32 _connectorWeight, uint256 _depositAmount) public view returns (uint256) {
        // validate input
        require(_supply > 0 && _connectorBalance > 0 && _connectorWeight > 0 && _connectorWeight <= MAX_WEIGHT, "calculatePurchaseReturn failed");
    ...
    */
    function calculateSaleReturn(uint256 _supply, uint256 _connectorBalance, uint32 _connectorWeight, uint256 _sellAmount) public view returns (uint256) {
        // validate input
        require(_supply > 0 && _connectorBalance > 0 && _connectorWeight > 0 && _connectorWeight <= MAX_WEIGHT && _sellAmount <= _supply, "calculateSaleReturn failed");
    ...
    */
    ...
    function maxExpArraySearch(uint8 _val) internal view returns(uint256) {
        uint256 value = expArray.searchArray(_val);
        return value;
    }
    /**
    ...
    */
    function findPositionInMaxExpArray(uint256 _x) internal view returns (uint8) {
        ...
        while (lo + 1 < hi) {
            uint8 mid = (lo + hi) / 2;
            if (maxExpArraySearch(mid) >= _x)
                lo = mid;
            else
                hi = mid;
        }

        if (maxExpArraySearch(hi) >= _x)
            return hi;
        if (maxExpArraySearch(lo) >= _x)
            return lo;

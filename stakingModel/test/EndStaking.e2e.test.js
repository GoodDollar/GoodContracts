...
contract("Contracts inactivation network e2e tests - reserve, fundmanager, pool and ubi", ([founder]) => {
  ...
  let deploy_settings, ubiBridgeRecipient, avatarAddress, registrar, absoluteVote, proposalId, addMinter, endUbiPool, endReserve, endFundManager, endUbi;
  ...
  before(async function() {
    ...
    await cDAI.transfer(goodReserve.address, (Math.floor(cdaibalance.toNumber() / 2)).toString());
    await cDAI.transfer(goodFundManager.address, (Math.floor(cdaibalance.toNumber() / 2)).toString());
    await goodDollar.transfer(firstClaimPool.address, (Math.floor(gdbalance.toNumber() / 2)).toString());
    await goodDollar.transfer(goodFundManager.address, (Math.floor(gdbalance.toNumber() / 2)).toString());
  });
  ...
  it("should be able to make the reserve contract inactive and the avatar should recieved the funds from the contract", async () => {
    ...
    await proposeAndRegister(endReserve.address,
    registrar, proposalId, absoluteVote, avatarAddress, founder);
    ...
    expect(avatarbalance2.sub(avatarbalance1).toString()).to.be.equal(contractbalance.toString());
  });
  ...
  it("should be able to make the fundmanager contract inactive and the avatar should recieved the funds from the contract", async () => {
    ...
    await proposeAndRegister(endFundManager.address,
    registrar, proposalId, absoluteVote, avatarAddress, founder);
    ...
    expect(code.toString()).to.be.equal("0x");
    expect(avatarcdaibalance2.sub(avatarcdaibalance1).toString()).to.be.equal(contractcdaibalance.toString());
    expect(avatargdbalance2.sub(avatargdbalance1).toString()).to.be.equal(contractgdbalance.toString());
  });
  ...
  it("should be able to make the pool contract inactive and the avatar should recieved the funds from the contract", async () => {
    ...
    await proposeAndRegister(endUbiPool.address,
    registrar, proposalId, absoluteVote, avatarAddress, founder);
  ...
  it("should be able to make the ubi contract inactive and the avatar should recieved the funds from the contract", async () => {
    ...
    let avatarbalance1 = await goodDollar.balanceOf(avatarAddress);
    await proposeAndRegister(endUbi.address,
    registrar, proposalId, absoluteVote, avatarAddress, founder);
    await endUbi.end();
    let avatarbalance2 = await goodDollar.balanceOf(avatarAddress);

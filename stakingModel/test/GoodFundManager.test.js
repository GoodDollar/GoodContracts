...
contract("GoodFundManager - transfer interest from the staking contract to the reserve contract", ([founder, staker, homeAvatar]) => {
  ...
  before(async () => {
    ...
    goodFundManager = await GoodFundsManager.new(cDAI.address, BLOCK_INTERVAL, avatar.address, identity.address, bridge.address, homeAvatar);
  ...
  it("should transfer ubi interest to the reserve and recieves minted gd back to the staking contract", async () => {
    ...
    expect(homeAvatarAfter.toString()).to.be.equal(gdUBI.toString());
  ...
  it("should destroy the contract and transfer funds to the given destination", async () => {
    ...
    let fundmanagerGDBalanceBefore = await goodDollar.balanceOf(goodFundManager.address);
    let encodedCall = web3.eth.abi.encodeFunctionCall({
      name: 'end',
      type: 'function',
      inputs: [{
    ...
    expect((avatarCDAIBalanceAfter - avatarCDAIBalanceBefore).toString()).to.be.equal(fundmanagerCDAIBalanceBefore.toString());
    expect(fundmanagerCDAIBalanceAfter.toString()).to.be.equal("0");
    expect((avatarGDBalanceAfter - avatarGDBalanceBefore).toString()).to.be.equal(fundmanagerGDBalanceBefore.toString());

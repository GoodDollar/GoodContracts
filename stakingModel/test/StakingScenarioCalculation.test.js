...
const MarketMaker = artifacts.require("GoodMarketMakerMock");
const GoodFundsManager = artifacts.require("GoodFundManager");
const GoodCompoundStakingMock = artifacts.require("GoodCompoundStaking");
...
async function resetGDRate(
  ...
  await goodDollar.burn(burnVal, {from:ubirecipient});
  await marketMaker.resetGDRate(cdaiAdd);
}

contract("InterestDistribution - Scenario based calculations for staking model", ([S1, S2, S3, ubirecipient]) => {
  let dai,
      cDAI,
      marketMaker,
      goodReserve,
    ...
    before(async () => {
      ...
      await dai.mint(S1, web3.utils.toWei("1000", "ether"));
      await dai.mint(S2, web3.utils.toWei("1000", "ether"));
      await dai.mint(S3, web3.utils.toWei("1000", "ether"));
      await dai.approve(simpleStaking.address, web3.utils.toWei("10000", "ether"), {
        from: S1
      });
      await dai.approve(simpleStaking.address, web3.utils.toWei("10000", "ether"), {
        from: S2
      ...
      await goodDollar.approve(goodReserve.address, 1e8,{from:ubirecipient});
    });
  describe('Multiple Staker stakes right after interest cycle, no withdrawal', function() {
    ...
    it("Staker 1 stakes 100 Token and donate 20% of interest", async () => {
      ...
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.4", "ether"))
          .catch(e => e);

      let beforeDAIBal = await dai.balanceOf(S1);

      /**
        * S1 stakes 100DAI, donating 20%
        */
        await simpleStaking.stake(web3.utils.toWei("100", "ether"), 20, {
          from: S1
        });


        let afterDAIBal = await dai.balanceOf(S1);

        expect((beforeDAIBal - afterDAIBal)).to.be.equal((web3.utils.toWei("100", "ether"))/1); 


        let stakerData = await simpleStaking.getStakerData(S1);
        let interestData = await simpleStaking.interestData();
        let yieldData = await simpleStaking.getYieldData(S1);
        ...
        */
        expect((await cDAI.balanceOf(simpleStaking.address)).toString()).to.be.equal(web3.utils.toWei("25", "gwei")); 

        // 1st stake, so no interest generated yet
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("0");

        // 1st stake, so no interest generated yet
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("0"); 
    });
    ...
    it("Staker 2 stakes 50 Token and donate 50% of interest", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.5", "ether"))
          .catch(e => e);

      let beforeDAIBal = await dai.balanceOf(S2);
      ...
      /**
        ...
        */
        await simpleStaking.stake(web3.utils.toWei("50", "ether"), 50, {
          from: S2
        });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 612);

        let afterDAIBal = await dai.balanceOf(S2);

        expect((beforeDAIBal - afterDAIBal)).to.be.equal((web3.utils.toWei("50", "ether"))/1); 

        let stakerData = await simpleStaking.getStakerData(S2);
        let interestData = await simpleStaking.interestData();
        let yieldData = await simpleStaking.getYieldData(S2);
        ...
        */      
        ...
        expect((interestData[4]).toString()).to.be.equal(web3.utils.toWei("5", "gwei")); 
        ...
        */ 
        expect((await cDAI.balanceOf(simpleStaking.address)).toString()).to.be.equal(web3.utils.toWei("30", "gwei")); 
        ...
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("200000");
        ...
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("800000"); 
    });
    ...
    it("Staker 3 stakes 360 Token and donate 40% of interest", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.6", "ether"))
          .catch(e => e);

      let beforeDAIBal = await dai.balanceOf(S3);
      /**
        ...
        * newlyMintGD = 50 x 200 = 10,000
        */
        await simpleStaking.stake(web3.utils.toWei("360", "ether"), 40, {
          from: S3
        });
        ...
        let afterDAIBal = await dai.balanceOf(S3);

        expect((beforeDAIBal - afterDAIBal)).to.be.equal((web3.utils.toWei("360", "ether"))/1); 

        let stakerData = await simpleStaking.getStakerData(S3);
        let interestData = await simpleStaking.interestData();
        let yieldData = await simpleStaking.getYieldData(S3);
        ...
        */   
        ...
        expect((interestData[4]).toString()).to.be.equal(web3.utils.toWei("10", "gwei")); 
        ...
        */
        expect((await cDAI.balanceOf(simpleStaking.address)).toString()).to.be.equal(web3.utils.toWei("85", "gwei")); 
        ...
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("500000");
        ...
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("1500000");
    });
    ...
    it("Staker 1 again stakes 250 Token and donate 60% of interest", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.65", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        await simpleStaking.stake(web3.utils.toWei("250", "ether"), 60, {
          from: S1
        });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 801);

        let afterDAIBal = await dai.balanceOf(S1);

        expect((beforeDAIBal - afterDAIBal)).to.be.equal((web3.utils.toWei("250", "ether"))/1); 
        ...
        */     
        ...
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("165"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("1169"); 
        ...
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("984616");
        ...
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("2323076");
    });
    ...
    it("Collecting all GD for excess interest token holding", async () => {
      ...
      // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
      await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 1023);
      ...
      */      
      ...
      expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("248");  
      ...
      */
      expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("1085"); 
      ...
      */
      expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("1729671");
      ...
      */
      expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("3248350");
      ...
      */
      expect((await simpleStaking.calculateGDInterest(S1)).toString()).to.be.equal("1934065");
      /**
      * Max[25 x 214.28 - (2500 + 0),0] = Max[2857.14, 0] = 2857.14 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S2)).toString()).to.be.equal("285714");
      /**
      * Max[216 x 214.28 - (36000 + 0),0] = Max[10285.70, 0] = 10285.70 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S3)).toString()).to.be.equal("1028570");
    });
  });
  ...
  describe('Few stakers stake and Few staker withdraw Interest', function() {
    it("Staker 1 stakes 60 Token and donate 0% of interest", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.8", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        await simpleStaking.stake(web3.utils.toWei("60", "ether"), 0, {
          from: S1
        });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 1662);

        let afterDAIBal = await dai.balanceOf(S1);

        expect((beforeDAIBal - afterDAIBal)).to.be.equal((web3.utils.toWei("60", "ether"))/1); 
        ...
        */   
        ...
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("384"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("1025");
        ...
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("2940385");
        ...
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("4751921"); 
    });

    it("Staker 2 Withdraws their share of interest", async () => {

      let beforeGDBal = await goodDollar.balanceOf(S2);

      /**
        * S2 Withdraws interest
        * will not mint new GD because no interest is generted since last update.
        */
        await simpleStaking.withdrawGDInterest( {
          from: S2
        });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 0);

        let afterGDBal = await goodDollar.balanceOf(S2);

        expect((afterGDBal - beforeGDBal).toString()).to.be.equal("374999"); 
        ...
        */
        ...
        /*
        * Formula:
        * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked) - Withdrawn
        * 47519.21 + (0 x 481 / 820) = 47519.21 + 0 - 3749.99 = 43769.22 , precision points 2
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("4376922"); 
    });

    it("Staker 3 Withdraws their share of interest", async () => {

      let beforeGDBal = await goodDollar.balanceOf(S3);
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.9", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        ...
        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 1395);


        let afterGDBal = await goodDollar.balanceOf(S3);

        expect((afterGDBal - beforeGDBal).toString()).to.be.equal("2399998"); 
        ...
        */   
        ...
        expect((interestData[3]).toString()).to.be.equal("6088032");  
        // interestTokenEarnedToDate, 384.36 + 113.88
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("498"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("911");

        /*
        * Formula:
        * ubiGDBlance = ubiGDBlance + newGDMinted - interestGD
        * 29403.85 + 22777.77 - 13361.11 = 38820.51 , precision points 2
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("3882051");

        /*
        * Formula:
        * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked) - withdrawn
        * 43769.22 + (481 x 22777.77 / 820) - 23999.98 = 33130.35 , precision points 2
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("3313035"); 
    });
    ...
    it("Staker 2 stakes 100 Token and donate 30% of interest", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("0.95", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        await simpleStaking.stake(web3.utils.toWei("100", "ether"), 30, {
          from: S2
        });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 587);
        ...
        */      
        ...
        // gdInterestEarnedToDate, 60880.32 + 5625.73 (481 x 9590.643 / 820)
        expect((interestData[3]).toString()).to.be.equal("6650605");  
        // interestTokenEarnedToDate, 498.24 + 47.95
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("546"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("968");

        /*
        * Formula:
        * ubiGDBlance = ubiGDBlance + newGDMinted - interestGD
        * 38820.52 + 9590.643 - 5625.73 = 42785.42 , precision points 2
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("4278542");

        /*
        * Formula:
        * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
        * 33130.35 + (9590.643 x 481 / 820) = 38756.08 , precision points 2
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("3875608");  
    });
    ...
    it("Collecting all GD for excess interest token holding", async () => {
      ...
      */      
      ...
      // globalTotalEffectiveStake
      expect((interestData[2]).toString()).to.be.equal(web3.utils.toWei("551", "ether")); 
      // gdInterestEarnedToDate, 66506.05 + 0 (66506.05 + 551 x 0 / 920)
      expect((interestData[3]).toString()).to.be.equal("6650605");  
      ...
      * Formula:
      * ubiGDBlance = ubiGDBlance + newGDMinted - interestGD
      * 42785.42 + 0 - 0 = 42785.42 , precision points 2
      */
      expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("4278542");
      ...
      * Formula:
      * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
      * 38756.08 + (0 x 551 / 920) = 38756.08 + 0 = 38756.08 , precision points 2
      */
      expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("3875608");
      ...
      */
      /**
      * Max[240 x 289.47 - (34230.76923 + 0),0] = Max[35242.90, 0] = 35242.90 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S1)).toString()).to.be.equal("3524290");
      /**
      * Max[95 x 289.47 - (22763.157 + 3750),0] = Max[986.85, 0] = 986.85 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S2)).toString()).to.be.equal("98685");
      /**
      * Max[216 x 289.47 - (36000 + 24000),0] = Max[2526.32, 0] = 2526.32 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S3)).toString()).to.be.equal("252632");
    });
  });
  ...
  describe('No one stakes in this interest cycle but Itoken rate will changes so some interest will be generated', function() {
    it("Collecting all GD for excess interest token holding", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("1", "ether"))
          .catch(e => e);
      ...
      // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
      await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 593);
      ...
      */      
      ...
      expect((interestData[3]).toString()).to.be.equal("7230604");  
      // interestTokenEarnedToDate, 546.19 + 48.42
      expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("594"); 
      ...
      */
      expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("920"); 

      /*
      * Formula:
      * ubiGDBlance = ubiGDBlance + newGDMinted - interestGD
      * 42785.44 + 9684.21 - 5800 = 46669.64 , precision points 2
      */
      expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("4666964");

      /*
      * Formula:
      * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
      * 38756.08 + (9684.21 x 551 / 920) = 38756.08 + 5799.99 = 44556.07 , precision points 2
      */
      expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("4455607");
      ...
      */
      /**
      * Max[240 x 299.99 - (34230.76923 + 0),0] = Max[37769.20, 0] = 37769.21 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S1)).toString()).to.be.equal("3776921");
      /**
      * Max[95 x 299.99 - (22763.157 + 3750),0] = Max[1986.84, 0] = 1986.84 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S2)).toString()).to.be.equal("198684");
      /**
      * Max[216 x 299.99 - (36000 + 24000),0] = Max[4800.00, 0] = 4800 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S3)).toString()).to.be.equal("480000");
    });
  });
  ...
  describe('Few stakers stakes and few staker withdraw Interest and stake', function() {
    it("Staker 1 Withdraws partial stake worth 150 Token", async () => {
      ...
      let beforeGDBal = await goodDollar.balanceOf(S1);
      let beforeDAIBal = await dai.balanceOf(S1);
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("1.05", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        await simpleStaking.withdrawStake(web3.utils.toWei("150", "ether"), {
          from: S1
          });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 537);

        let afterGDBal = await goodDollar.balanceOf(S1);
        let afterDAIBal = await dai.balanceOf(S1);
        ...
        * Formula: 
        * EarnedGDInterest = MAX[TotalEfectiveStaked x AccumulatedYieldPerDAI - (StakeBuyinRate + WithdrawnToDate), 0]
        * Max[240 x 309.52 - (34230.76923 + 0),0] = Max[40054.92, 0] = 40054.92 , precision points 2
        */
        expect((afterGDBal - beforeGDBal).toString()).to.be.equal("4005492");

        expect((afterDAIBal - beforeDAIBal).toString()).to.be.equal(web3.utils.toWei("150", "ether"));
        ...
        */   
        ...
        // gdInterestEarnedToDate, 77553.65 (72306.02 + 551 x 8761.904762 / 920)
        expect((interestData[3]).toString()).to.be.equal("7755365");  
        // interestTokenEarnedToDate, 594.61 + 43.80
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("638"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("733"); 

        /*
        * Formula:
        * ubiGDBlance = ubiGDBlance + newGDMinted - interestGD
        * 46669.66 + 8761.90 - 5247.61 = 50183.93 , precision points 2
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("5018393");

        /*
        * Formula:
        * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
        * 44556.07 + (8761.90 x 551 / 920) - 40054.92 = 9748.76 , precision points 2
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("974876"); 
    });
    ...
    it("Staker 2 Withdraws Entire stake worth 150 Token", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("1.1", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        ...
        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 408);
        ...
        * Formula: 
        * EarnedGDInterest = MAX[TotalEfectiveStaked x AccumulatedYieldPerDAI - (StakeBuyinRate + WithdrawnToDate), 0]
        * Max[95 x 318.18 - (22763.157 + 3750),0] = Max[3714.11, 0] = 3714.11 , precision points 2
        */
        expect((afterGDBal - beforeGDBal).toString()).to.be.equal("371411");
        ...
        */     
        ...
        // gdInterestEarnedToDate, 81563.99 (77553.63 + 463.2 x 6666.66 / 770)
        expect((interestData[3]).toString()).to.be.equal("8156399");  
        // interestTokenEarnedToDate, 638.41 + 33.32
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("672"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("563");
        ...
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("5284025");

        /*
        * Formula:
        * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
        * 9748.76 + (463.2 x 6666.66 / 770) -  3714.11 = 10044.99 , precision points 2
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("1004499"); 
    });
    ...
    it("Staker 3 stakes 100 Token and donate 40% of interest", async () => {
      ...
      // Updating the rate so we can see interest generating for staked tokens.
      await cDAI
          .setExchangeRate(web3.utils.toWei("1.15", "ether"))
          .catch(e => e);
      ...
      /**
        ...
        */
        await simpleStaking.stake(web3.utils.toWei("100", "ether"), 40, {
          from: S3
        });

        // Reseting the GD price to 1cDAI = 200 GD and burning extra GD minted in mintExpansion (It was not accounted in excel sheet)
        await resetGDRate(ubirecipient, cDAI.address, goodDollar, marketMaker, 300);
        ...
        */   
        ...
        expect((interestData[3]).toString()).to.be.equal("8447462");  
        // interestTokenEarnedToDate, 671.73 + 24.50
        expect((Math.floor(interestData[4]/1e8)).toString()).to.be.equal("696"); 
        ...
        */
        expect((Math.floor(await cDAI.balanceOf(simpleStaking.address)/1e8)).toString()).to.be.equal("626");
        ...
        */
        expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("5483080");

        
        /** Formula:
        * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
        * 10044.99 + (368.20 x 4901.18 / 620) = 12955.62 , precision points 2
        */
        expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("1295562"); 
    });
    ...
    it("Collecting all GD for excess interest token holding", async () => {
      ...
      */      
      ...
      // globalTotalEffectiveStake
      expect((Math.floor(interestData[2]/1e18)).toString()).to.be.equal("428"); 
      // gdInterestEarnedToDate, 84474.62 + 0 (8447459 + 428.2 x 0 / 760)
      expect((interestData[3]).toString()).to.be.equal("8447462");  
      ...
      */
      ...
      /** Formula:
      * ubiGDBlance = ubiGDBlance + newGDMinted - interestGD
      * 54830.83 + 0 - 0 = 54830.8 , precision points 2
      */
      expect((await goodDollar.balanceOf(ubirecipient)).toString()).to.be.equal("5483080");
      ...
      * Formula:
      * interestGDBalance = interestGDBalance + (newGDMinted x GlobalTotalEffectiveStake / globalTotalStaked)
      * 12955.62 + (0 x 428.2 / 760) = 12955.65 + 0 = 12955.62 , precision points 2
      */
      expect((await goodDollar.balanceOf(simpleStaking.address)).toString()).to.be.equal("1295562");
      ...
      */
      expect((await simpleStaking.calculateGDInterest(S1)).toString()).to.be.equal("252082");
      /**
      * Max[0 x 326.08 - (0 + 0),0] = Max[0, 0] = 0 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S2)).toString()).to.be.equal("0");
      /**
      * Max[276 x 326.08 - (55565.21739 + 24000),0] = Max[10434.77, 0] = 10434.77 , precision points 2
      */
      expect((await simpleStaking.calculateGDInterest(S3)).toString()).to.be.equal("1043477");
    });
  });

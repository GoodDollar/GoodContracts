...
contract("InterestDistribution - Scenario based calculations", ([S1, S2, S3, ubirecipient]) => {
  let dai,
      cDAI,
      marketMaker,
      goodReserve,
    ...
    before(async () => {
      ...
      await dai.mint(S1, web3.utils.toWei("10000", "ether"));
      await dai.mint(S2, web3.utils.toWei("10000", "ether"));
      await dai.mint(S3, web3.utils.toWei("10000", "ether"));
...
console.log(await goodDollar.balanceOf(ubirecipient));
  console.log(await goodDollar.balanceOf(simpleStaking.address));
  console.log((await marketMaker.currentPrice(cDAI.address))/1);

...
contract("Integration - Active Period", ([founder, claimer, nonClaimer]) => {
  ...
  before(async () => {
    ...
    await helpers.assertVMException(
      ActivePeriodMock.new(periodEnd, periodStart, avatar.address),
      "start cannot be after nor equal to end"
    );
    activePeriodMock = await ActivePeriodMock.new(periodStart, periodEnd, avatar.address);
  });

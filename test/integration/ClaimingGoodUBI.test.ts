...
const ExpArray = artifacts.require("ExpArray");
const BancorFormula = artifacts.require("BancorFormula");
const GoodUBI = artifacts.require("GoodUBI");

contract("Integration - Claiming GoodUBI", ([founder, claimer, nonClaimer]) => {
  ...
  let expArray: helpers.ThenArg<ReturnType<typeof ExpArray['new']>>;
  let bancorFormula: helpers.ThenArg<ReturnType<typeof BancorFormula['new']>>;
  let ubi: helpers.ThenArg<ReturnType<typeof GoodUBI['new']>>;
  ...
  before(async () => {
    ...
    const periodEnd = periodStart + periodOffset * 1000;
    ...
    expArray = await ExpArray.new()
    bancorFormula = await BancorFormula.new(expArray.address);

    ubi = await GoodUBI.new(avatar.address, identity.address, bancorFormula.address, web3.utils.toWei("1000"), periodStart, periodEnd);
  ...
  it("##TODO## should correctly calculate UBI ##TODO##", async () => {
  })

  it("##TODO## should correctly claim UBI ##TODO##", async () => {
  ...
  it("should end UBI period", async () => {
     ...
     await helpers.increaseTime(periodOffset * 1000);

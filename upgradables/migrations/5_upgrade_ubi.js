...
const UBIScheme = artifacts.require("UUBIScheme.sol");
...
module.exports = async function(deployer, network) {
  ...
  const ubi = await deployProxy(
    UBIScheme,
    [homedao.Avatar, homedao.Identity, ubiPool.address, 14, 8],
    {
      deployer,
  ...
  const ubi2 = await upgradeProxy(ubi.address, UBIScheme, {
    deployer,
    unsafeAllowCustomTypes: true
  });

  console.log("proposing UBI to DAO", ubi.address);
